<script>
	...
	let categoryLabel = $state(form?.categoryLabel ?? data.post.categoryLabel ?? '');
...
<div class="mx-auto max-w-6xl space-y-6">
	...
	<div class="flex items-center justify-between border-b border-slate-200 pb-4">
		<div class="flex items-center gap-4">
			...
			<div>
				<h1 class="text-xl font-bold tracking-wider text-slate-900 uppercase">Edit Post</h1>
				<p class="mt-1 text-[10px] font-bold tracking-wider text-slate-500 uppercase">
					{data.post.published ? 'Published' : 'Draft'} • Created {new Date(
						data.post.createdAt
					).toLocaleDateString()}
				</p>
			</div>
		...
		<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
			...
			<div class="space-y-6">
				...
				<div class="border border-slate-200 bg-white p-5 shadow-sm">
					...
					<div class="space-y-3">
						...
						<button
							...
							value="save"
							disabled={loading}
							class="flex w-full items-center justify-center gap-2 bg-slate-100 px-4 py-3 text-xs font-bold tracking-wider text-slate-700 uppercase transition-colors hover:bg-slate-200"
						>
							{#if loading}
								<span
								...
								></span>
							{/if}
							{data.post.published ? 'Update Post' : 'Save Draft'}
						</button>
						{#if data.post.published}
							<button
								type="submit"
								name="action"
								value="unpublish"
								class="w-full bg-amber-100 px-4 py-3 text-xs font-bold tracking-wider text-amber-700 uppercase transition-colors hover:bg-amber-200"
							>
								Unpublish
							</button>
						{/if}
				...
				<div class="border border-red-200 bg-white p-5 shadow-sm">
					...
					<p class="mb-4 text-xs text-slate-500">Once deleted, this post cannot be recovered.</p>

					<button
						type="submit"
						formaction="?/delete"
						class="flex w-full items-center justify-center gap-2 border border-red-200 bg-red-50 px-4 py-2.5 text-xs font-bold tracking-wider text-red-600 uppercase transition-colors hover:bg-red-100 hover:text-red-700"
					>
